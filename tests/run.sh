...
echo "[1/11] linter passes on the real repo"
...
echo "[2/11] linter rejects the bad fixture for the right reason"
...
echo "[3/11] hooks behave (valid JSON output + guard inert when off)"
...
echo "[4/11] ledger + audit behavior"
...
echo "[5/11] run-report behavior + parity"
...
echo "[6/11] evolve detection + apply"
...
echo "[7/11] evolve follow-ups: --since + --explore"
...
echo "[8/11] loop replay behavior"
