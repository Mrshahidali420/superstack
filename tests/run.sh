...
echo "[1/18] linter passes on the real repo"
...
echo "[2/18] linter rejects the bad fixture for the right reason"
...
echo "[3/18] hooks behave (valid JSON output + guard inert when off)"
...
echo "[4/18] ledger + audit behavior"
...
echo "[5/18] run-report behavior + parity"
...
echo "[6/18] evolve detection + apply"
...
echo "[7/18] evolve follow-ups: --since + --explore"
...
echo "[8/18] loop replay behavior"
...
echo "[15/18] ctx shrink + retrieval"
...
echo "[18/18] panel behavior"
if bash "$ROOT/tests/panel.test.sh" >/dev/null 2>&1; then
  echo "      PASS"
else
  echo "      FAIL - panel suite"; fail=1
fi
