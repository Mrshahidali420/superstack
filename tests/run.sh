...
echo "[1/17] linter passes on the real repo"
...
echo "[2/17] linter rejects the bad fixture for the right reason"
...
echo "[3/17] hooks behave (valid JSON output + guard inert when off)"
...
echo "[4/17] ledger + audit behavior"
...
echo "[5/17] run-report behavior + parity"
...
echo "[6/17] evolve detection + apply"
...
echo "[7/17] evolve follow-ups: --since + --explore"
...
echo "[8/17] loop replay behavior"
...
echo "[15/17] ctx shrink + retrieval"
